...
import { ExternalLink, ImageIcon, Loader2, Package, RefreshCw } from "lucide-react";
              ...
              {gallery!.screenshots.map((shot) => (
                <button
                  ...
                  onClick={() => setSelectedShot(
                    selectedShot === shot.dataUrl ? null : shot.dataUrl
                  )}
                  className={clsx(
                  ...
                  title={shot.name}
                >
                  <img src={shot.dataUrl} alt={shot.name} className="w-full h-full object-contain bg-black/30" />
                </button>
              ))}

              {selectedShot && (
                <div className="flex-shrink-0 rounded-lg border border-surface-600 overflow-hidden bg-surface-950 w-auto">
                  <img
                    src={selectedShot}
                    alt="Selected screenshot"
                    className="h-28 w-auto object-contain bg-black/30"
                  />
                </div>
              )}
            </div>
  ...
  return (
    <div className="flex flex-col h-full border-l border-surface-600 bg-surface-900/60 min-w-0">
      ...
      <div className="flex-1 overflow-auto p-3 space-y-3">
        ...
        {selectedShot && (
          <div className="rounded-lg border border-surface-600 overflow-hidden bg-surface-950">
            <img
              src={selectedShot}
          ...
          <div className="grid grid-cols-3 gap-2">
            {gallery!.screenshots.map((shot) => (
              <button
                key={shot.name}
                type="button"
                onClick={() => setSelectedShot(shot.dataUrl)}
                className={clsx(
                  "rounded border overflow-hidden aspect-video bg-surface-950 hover:border-violet-500/50 transition-colors",
                  selectedShot === shot.dataUrl
                    ? "border-violet-500 ring-1 ring-violet-500/40"
                    : "border-surface-600"
                )}
